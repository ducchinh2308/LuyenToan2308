...
//// =====================================================================
//// Hàm 8.2: Xử lý Tab "Nhiệm Vụ Trên Lớp" (NÂNG CẤP PHÂN TÁCH TAB CẦN LÀM / ĐÃ LÀM)
//// =====================================================================
//async function ham_8_2_tab_nhiem_vu_bat_buoc() {
//    const vungLamViec = document.getElementById('vung-lam-viec-hoc-sinh');
//    vungLamViec.innerHTML = `<div style="text-align: center; padding: 40px;"><h3 style="color:#28a745;">⏳ Đang tổng hợp bài tập và tiến độ...</h3></div>`;
...
//        // 3. TRUY VẤN KẾT QUẢ ĐỂ LẤY ĐIỂM VÀ SỐ LƯỢT LAM BÀI CỦA HS
//        let demSoLuotLam = {};
//        let ketQuaGanNhat = {};
...
//        // 5. PHÂN LOẠI CHI TIẾT THEO TIÊU CHÍ MỚI: CẦN LÀM vs ĐÃ LÀM
//        const now = new Date();
//        let dsCanLam = [], dsDaLam = [];
...
//            // Kiểm tra xem bài tập này đã từng được làm lần nào chưa
//            if (soLuotDaLam > 0) {
//                // Đã làm ít nhất 1 lần -> Đẩy thẳng vào kho ĐÃ LÀM
//                dsDaLam.push(nv);
//            } else {
//                // Chưa làm lần nào -> Nằm ở kho CẦN LÀM (bao gồm cả bài sắp mở ngầm)
//                dsCanLam.push(nv);
//            }
//        });

//        // HÀM PHỤ TÍNH THỜI GIAN THÔNG MINH
//        const tinhKhoangCachThoiGian = (targetDate, isMo) => {
//            if (!targetDate) return "";
//            const diff = targetDate.getTime() - now.getTime();
//            const absDiff = Math.abs(diff);
...
//        // HÀM VẼ THẺ CARD NHIỆM VỤ (Đã tinh chỉnh layout nút bấm tinh tế)
//        const renderCard = (nv) => {
//            const tMo = anToanThoiGian(nv.thoi_gian_mo);
//            const tDong = anToanThoiGian(nv.thoi_gian_dong);
//            const tTao = nv.ngay_tao;
...
//            const daChuaDenGioMo = (tMo && now.getTime() < tMo.getTime());
//            const daQuaHan = (tDong && now.getTime() > tDong.getTime());
//            const daHetLuot = (gioiHanLuot > 0 && soLuotDaLam >= gioiHanLuot);

//            // Thiết lập màu sắc khung cảnh báo dựa trên tính chất thời gian
//            let mauVien = "#28a745", mauNenTag = "#f4fdf6";
//            if (daChuaDenGioMo) { mauVien = "#ffc107"; mauNenTag = "#fffbf0"; }
//            else if (daQuaHan || daHetLuot) { mauVien = "#dc3545"; mauNenTag = "#fff5f6"; }

//            // Thẻ HUD hiển thị điểm số nếu đã làm bài
//            const kqLatest = ketQuaGanNhat[nv.ma_nhiem_vu];
//            let htmlKetQua = "";
//            if (soLuotDaLam > 0 && kqLatest) {
//                htmlKetQua = `
...
//            // MẠCH XỬ LÝ NÚT HÀNH ĐỘNG THÔNG MINH ĐÚNG YÊU CẦU CỦA THẦY
//            let nutHanhDong = "";
//            let cssLuot = daHetLuot ? "color: #dc3545; font-weight: bold; background: #fff5f5; border: 1px solid #f5c6cb;" : "background: #e9ecef; color: #495057;";

//            if (daChuaDenGioMo) {
//                nutHanhDong = `<button disabled style="width: 100%; padding: 12px; background: #e9ecef; color: #6c757d; border: none; border-radius: 8px; font-weight: bold; cursor: not-allowed;">⏳ Đợi đến giờ mở</button>`;
//            }
//            else if (daQuaHan || daHetLuot) {
//                // Trường hợp đã khóa do quá hạn hoặc hết lượt -> Hiện nút Cứu trợ / Xin thêm lượt
//                const tenNhiemVuAnToan = (nv.ten_nhiem_vu || "Nhiệm vụ").replace(/'/g, "\\'");
//                let textNutXin = "🙋 XIN NỘP BÀI QUÁ HẠN";
//                let maLoaiXin = "QUA_HAN";
...
//                nutHanhDong = `
//                    <button onclick="ham_8_15_xin_luot_lam_bai('${nv.ma_nhiem_vu}', '${tenNhiemVuAnToan}', '${maLoaiXin}')"
//                            style="width: 100%; padding: 12px; background: #fd7e14; color: white; border: none; border-radius: 8px; font-weight: bold; cursor: pointer; transition: 0.2s; box-shadow: 0 2px 4px rgba(253,126,20,0.3);"
//                            onmouseover="this.style.background='#e67e22'" onmouseout="this.style.background='#fd7e14'">
//                        ${textNutXin}
//                    </button>
//                `;
//            }
//            else {
//                // Trường hợp ĐANG MỞ bình thường
//                if (soLuotDaLam > 0) {
//                    // Đã làm mà vẫn còn lượt (Nằm ở Tab ĐÃ LÀM) -> Hiện nút LÀM LẠI để cải thiện điểm
//                    nutHanhDong = `<button onclick="ham_8_7_cua_an_ninh('${nv.ma_nhiem_vu}')" style="width: 100%; padding: 12px; background: #00b4d8; color: white; border: none; border-radius: 8px; cursor: pointer; font-size: 15px; font-weight: bold; box-shadow: 0 2px 5px rgba(0,180,216,0.3);">🔄 LÀM LẠI LẦN NỮA</button>`;
//                } else {
//                    // Chưa làm lần nào (Nằm ở Tab CẦN LÀM) -> Hiện nút VÀO LÀM BÀI nguyên bản xanh lá
//                    nutHanhDong = `<button onclick="ham_8_7_cua_an_ninh('${nv.ma_nhiem_vu}')" style="width: 100%; padding: 12px; background: #28a745; color: white; border: none; border-radius: 8px; cursor: pointer; font-size: 15px; font-weight: bold; box-shadow: 0 2px 5px rgba(40,167,69,0.3);">🚀 VÀO LÀM BÀI</button>`;
//                }
//            }

//            return `
//                <div class="card-nhiem-vu" style="background: white; border: 1px solid #e0e0e0; border-top: 4px solid ${mauVien}; border-radius: 12px; padding: 18px; box-shadow: 0 4px 8px rgba(0,0,0,0.03); transition: transform 0.2s; max-width: 100%; box-sizing: border-box;" onmouseover="this.style.transform='translateY(-2px)'" onmouseout="this.style.transform='translateY(0)'">
//                    <div style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 10px; gap: 10px;">
//                        <h4 style="margin: 0; color: #2c3e50; font-size: 15px; line-height: 1.4; font-weight: bold;">${nv.ten_nhiem_vu}</h4>
//                        <span style="font-size: 10px; padding: 4px 8px; background: ${mauNenTag}; border: 1px solid ${mauVien}30; border-radius: 4px; color: ${mauVien}; white-space: nowrap; font-weight: bold; text-transform: uppercase;">
//                            ${nv.loai_nhiem_vu.split(' ')[0]}
//                        </span>
//                    </div>

//                    <div style="background: #f8f9fa; border-radius: 8px; padding: 10px; margin-bottom: 12px; font-size: 12px; color: #555; border: 1px solid #e9ecef; line-height: 1.5;">
//                        <div>🎯 <b>Lớp học:</b> <span style="color: #1a73e8; font-weight:bold;">${tenLopHienThi}</span></div>
//                        <div>👤 <b>Giáo viên:</b> <span style="font-weight: 500;">${tenGV}</span></div>
//                        <div style="font-size: 11px; color:#7f8c8d; margin-top: 2px;">🕒 Tạo từ: ${thoiGianTroiQua(tTao)}</div>
//                    </div>

//                    <div style="display: flex; gap: 6px; flex-wrap: wrap; margin-bottom: 12px;">
//                        <span style="font-size: 11px; background: #f1f3f5; color: #495057; padding: 3px 8px; border-radius: 4px; font-weight: 500;">
//                            ⏱️ ${nv.thoi_gian_lam_bai > 0 ? nv.thoi_gian_lam_bai + ' phút' : 'Không giới hạn'}
//                        </span>
//                        <span style="font-size: 11px; background: #f1f3f5; color: #495057; padding: 3px 8px; border-radius: 4px; font-weight: 500;">
//                            📦 ${nv.cau_truc_de || 'Đề tự do'}
//                        </span>
//                        <span style="font-size: 11px; padding: 3px 8px; border-radius: 4px; ${cssLuot}">
//                            🔄 Lượt: <b>${soLuotDaLam} / ${textLuotChoPhep}</b>
//                        </span>
//                    </div>

//                    <div style="margin-bottom: 15px; font-size: 12px; border-top: 1px solid #f1f3f5; padding-top: 10px; line-height: 1.6;">
//                        <div><span style="color: #28a745; font-weight: bold;">🟢 MỞ:</span> ${fTime(tMo)} <span style="color: #7f8c8d; font-size: 11px; font-style: italic;">${tMo ? tinhKhoangCachThoiGian(tMo, true) : ""}</span></div>
//                        <div><span style="color: #dc3545; font-weight: bold;">🔴 ĐÓNG:</span> ${fTime(tDong)} <span style="color: #d35400; font-size: 11px; font-weight: bold; background: #fff3cd; padding: 1px 4px; border-radius: 3px;">${tDong ? tinhKhoangCachThoiGian(tDong, false) : ""}</span></div>
//                    </div>
...
//        // 6. RÁP BỘ KHUNG GIAO DIỆN TAB ĐIỀU HƯỚNG MỚI CHO HỌC SINH
//        vungLamViec.innerHTML = `
//            <div style="display: flex; border-bottom: 2px solid #dee2e6; margin-bottom: 20px; gap: 5px; background: #fff; padding: 5px 5px 0 5px; border-radius: 8px 8px 0 0;">
//                <button id="btn-tab-can-lam" onclick="ham_8_x_switch_sub_tab('CAN_LAM')" style="padding: 12px 25px; border: none; background: #28a745; color: white; font-weight: bold; font-size: 14px; border-radius: 6px 6px 0 0; cursor: pointer; transition: 0.2s; display: flex; align-items: center; gap: 8px;">
//                    🎯 CẦN LÀM <span style="background: rgba(255,255,255,0.3); color: white; padding: 2px 8px; border-radius: 10px; font-size: 11px;">${dsCanLam.length}</span>
//                </button>
//                <button id="btn-tab-da_lam" onclick="ham_8_x_switch_sub_tab('DA_LAM')" style="padding: 12px 25px; border: none; background: transparent; color: #495057; font-weight: bold; font-size: 14px; border-radius: 6px 6px 0 0; cursor: pointer; transition: 0.2s; display: flex; align-items: center; gap: 8px;">
//                    ✅ ĐÃ LÀM <span style="background: #e9ecef; color: #495057; padding: 2px 8px; border-radius: 10px; font-size: 11px;">${dsDaLam.length}</span>
//                </button>
//            </div>

//            <div id="vung-chua-cards-nhiem-vu" style="min-height: 200px;">
//                </div>
//        `;

//        // Đóng gói mảng HTML chuỗi card để hàm switch-tab bốc vẽ ngầm không cần gọi lại database
//        window.CachedCardsCanLamHtml = dsCanLam.length === 0
//            ? '<div style="text-align:center; color:#7f8c8d; padding: 40px; font-style:italic; background:white; border-radius:8px; border:1px dashed #ccc;">🥳 Tuyệt vời! Em đã hoàn thành sạch sẽ tất cả nhiệm vụ được giao!</div>'
//            : `<div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(320px, 1fr)); gap: 20px;">${dsCanLam.map(nv => renderCard(nv)).join('')}</div>`;

//        window.CachedCardsDaLamHtml = dsDaLam.length === 0
//            ? '<div style="text-align:center; color:#7f8c8d; padding: 40px; font-style:italic; background:white; border-radius:8px; border:1px dashed #ccc;">🍃 Em chưa nộp bài tập nào. Hãy sang bên mục "CẦN LÀM" để chiến đấu nhé!</div>'
//            : `<div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(320px, 1fr)); gap: 20px;">${dsDaLam.map(nv => renderCard(nv)).join('')}</div>`;

//        // Mặc định ban đầu kích hoạt vẽ luôn Tab CẦN LÀM
//        window.ham_8_x_switch_sub_tab('CAN_LAM');

//    } catch (error) {
//        vungLamViec.innerHTML = `<div style="color: red; text-align: center; padding: 20px; font-weight:bold;">❌ Lỗi hệ thống: ${error.message}</div>`;
//    }
//}
...
//window.ham_8_x_switch_sub_tab = function (loaiTab) {
//    const btnCanLam = document.getElementById('btn-tab-can-lam');
//    const btnDaLam = document.getElementById('btn-tab-da_lam');
//    const vungChua = document.getElementById('vung-chua-cards-nhiem-vu');

//    if (!vungChua || !btnCanLam || !btnDaLam) return;

//    if (loaiTab === 'CAN_LAM') {
//        // Active Tab Cần làm (Màu Xanh lá cây nguyên bản của thầy)
//        btnCanLam.style.background = '#28a745';
//        btnCanLam.style.color = 'white';

//        // Deactive Tab Đã làm
//        btnDaLam.style.background = 'transparent';
//        btnDaLam.style.color = '#495057';

//        vungChua.innerHTML = window.CachedCardsCanLamHtml;
//    } else {
//        // Active Tab Đã làm (Màu Xanh ngọc bích cực ngầu)
//        btnDaLam.style.background = '#00b4d8';
//        btnDaLam.style.color = 'white';

//        // Deactive Tab Cần làm
//        btnCanLam.style.background = 'transparent';
//        btnCanLam.style.color = '#495057';

//        vungChua.innerHTML = window.CachedCardsDaLamHtml;
//    }
//};
...
//    // 🌟 ĐÃ XÓA KHỐI HIỂN THỊ STICKY ĐIỂM SỐ CHE MÀN HÌNH
//    // Trả lại tiêu đề đơn giản, gọn gàng cho khu vực đọc đề
//    let htmlContentRight = '';
//    if (phien.isLiveQuiz) {
...
//    // ==============================================================
//    // 🌟 KHỐI HIỂN THỊ ĐIỂM SỐ & PIN CHUYỂN SANG CỘT TRÁI (CHỈ HIỆN KHI LIVE QUIZ)
//    // ==============================================================
//    let htmlLiveDiem = '';
//    if (phien.isLiveQuiz) {
//        htmlLiveDiem = `
...
//    // ==============================================================
//    // 🌟 KÍCH HOẠT CHÍNH XÁC KATEX / MATHJAX & TIKZ
//    // ==============================================================
//    const vungCuon = document.getElementById('khu-vuc-cuon-de');
//    if (window.renderMathInElement) {
//        window.renderMathInElement(vungCuon, {
