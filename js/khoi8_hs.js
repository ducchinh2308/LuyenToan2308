...
// =====================================================================
// Hàm 8.6.3: Giao diện Làm bài thi Live (Load đề & Vẽ giao diện)
// =====================================================================
window.ham_8_6_3_bat_dau_lam_bai_live = async function () {
    const vungLamViec = document.getElementById('vung-lam-viec-hoc-sinh');
    vungLamViec.innerHTML = `<div style="text-align: center; padding: 50px;"><div style="font-size:40px; animation: spin 1s linear infinite;">⏳</div><h3>Đang tải dữ liệu đề thi...</h3></div>`;

    try {
        const maNV = window.ThongTinLiveHocSinh.maNhiemVu;

        // 1. Tải thông tin Nhiệm vụ và Học liệu (Đề thi)
        const { data: nv, error: errNV } = await _supabase.from('nhiem_vu').select('ma_hoc_lieu, ten_nhiem_vu').eq('ma_nhiem_vu', maNV).single();
        if (errNV) throw errNV;

        const { data: hl, error: errHL } = await _supabase.from('hoc_lieu').select('danh_sach_cau_hoi').eq('ma_hoc_lieu', nv.ma_hoc_lieu).single();
        if (errHL) throw errHL;

        // 2. Parse mảng câu hỏi
        let dsCauHoi = [];
        try { dsCauHoi = typeof hl.danh_sach_cau_hoi === 'string' ? JSON.parse(hl.danh_sach_cau_hoi) : hl.danh_sach_cau_hoi; } catch (e) { }

        if (!dsCauHoi || dsCauHoi.length === 0) throw new Error("Đề thi trống, không có câu hỏi nào!");

        // 3. Tính Tổng Trọng Số (Chuẩn hóa barem: TN=1, TLN=2, DS=4) để chấm điểm live
        let soTN = 0, soTLN = 0, soDS = 0;
        dsCauHoi.forEach(c => {
            const kieu = (c.kieuCau || c.kieu_cau || 'TN').toUpperCase();
            if (kieu === 'TN') soTN++;
            else if (kieu === 'TLN') soTLN++;
            else if (kieu === 'DS') soDS++;
        });
        window.HocSinhLiveState.tongTrongSo = (soTN * 1.0) + (soTLN * 2.0) + (soDS * 4.0) || 1;
        window.HocSinhLiveState.danhSachCauHoi = dsCauHoi;
        window.HocSinhLiveState.dapAnHocSinh = {}; // Khởi tạo rỗng

        // 4. VẼ GIAO DIỆN LÀM BÀI TRỰC TIẾP
        let htmlCauHoi = '';
        dsCauHoi.forEach((cau, index) => {
            const maCau = cau.ma_cau_hoi || cau.maCau;
            const kieuCau = (cau.kieuCau || cau.kieu_cau || 'TN').toUpperCase();
            const dapAnDung = cau.dap_an || cau.dapAn || '';
            const noiDung = cau.noi_dung || cau.noiDung || 'Nội dung câu hỏi bị thiếu...';

            htmlCauHoi += `
                <div style="background: white; border-radius: 12px; padding: 20px; margin-bottom: 20px; box-shadow: 0 4px 10px rgba(0,0,0,0.05); border: 1px solid #e0e0e0;">
                    <div style="font-weight: 900; color: #1a73e8; margin-bottom: 10px; font-size: 16px;">Câu ${index + 1}: <span style="font-size: 12px; color: #6c757d; font-weight: normal; background: #f1f3f4; padding: 2px 6px; border-radius: 4px;">(${kieuCau})</span></div>
                    <div style="font-size: 15px; color: #333; line-height: 1.6; margin-bottom: 15px; overflow-x: auto;">
                        ${noiDung}
                    </div>
            `;
            ...
            if (kieuCau === 'TN') {
                ['A', 'B', 'C', 'D'].forEach(da => {
                    htmlCauHoi += `
                        <label style="display: block; padding: 12px 15px; background: #f8f9fa; border: 2px solid #dee2e6; border-radius: 8px; margin-bottom: 8px; cursor: pointer; transition: 0.2s;" id="lbl_${maCau}_${da}">
                            <input type="radio" name="rad_${maCau}" value="${da}" style="margin-right: 10px; transform: scale(1.2);" 
                                   onchange="ham_8_6_4_chon_dap_an_live('${maCau}', '${da}', '${kieuCau}', '${dapAnDung}')"> 
                            <b style="font-size:15px;">${da}</b>
                        </label>
                    `;
                });
            }
            // RENDER TỰ LUẬN NGẮN HOẶC ĐÚNG/SAI
            else {
                htmlCauHoi += `
                    <input type="text" placeholder="Nhập đáp án của em vào đây..." style="width: 100%; padding: 12px; border: 2px solid #dee2e6; border-radius: 8px; font-size: 15px; box-sizing: border-box;" 
                           onchange="ham_8_6_4_chon_dap_an_live('${maCau}', this.value, '${kieuCau}', '${dapAnDung}')">
                `;
            }
        ...
        vungLamViec.innerHTML = `
            <div style="max-width: 800px; margin: 0 auto; padding-bottom: 80px;">
                <div style="background: #e74c3c; color: white; padding: 15px 20px; border-radius: 12px; margin-bottom: 20px; text-align: center; position: sticky; top: 10px; z-index: 100; box-shadow: 0 4px 15px rgba(231,76,60,0.4);">
                    <h3 style="margin: 0; font-size: 18px; font-weight: 900;">🔥 ĐANG THI ĐẤU: ${nv.ten_nhiem_vu}</h3>
                    <div style="font-size: 13px; margin-top: 5px;">Hãy làm bài nhanh nhất có thể. Bảng xếp hạng đang nhảy liên tục!</div>
                </div>

                ${htmlCauHoi}

                <button onclick="ham_8_6_5_nop_bai_live()" style="width: 100%; padding: 18px; background: #28a745; color: white; border: none; border-radius: 12px; font-weight: 900; font-size: 18px; cursor: pointer; transition: 0.2s; box-shadow: 0 4px 15px rgba(40,167,69,0.4);" onmouseover="this.style.background='#218838'" onmouseout="this.style.background='#28a745'">
                    ✅ NỘP BÀI VÀ XEM KẾT QUẢ
                </button>
            </div>
        `;

        // Render lại MathJax/LaTeX nếu có chứa công thức toán
        if (window.MathJax) { MathJax.typesetPromise(); }

    } catch (e) {
        vungLamViec.innerHTML = `<div style="color: #dc3545; text-align: center; padding: 20px;">❌ Lỗi tải đề thi: ${e.message}</div>`;
    }
}

// =====================================================================
// Hàm 8.6.4: Xử lý chấm điểm cục bộ SIÊU TỐC và Bắn sóng UPDATE lên Supabase
// =====================================================================
window.ham_8_6_4_chon_dap_an_live = async function (maCau, dapAnChon, kieuCau, dapAnDung) {
    // 1. Lưu tạm vào bộ nhớ RAM
    window.HocSinhLiveState.dapAnHocSinh[maCau] = dapAnChon.trim().toUpperCase();

    // Thay đổi màu sắc giao diện (tùy chọn UI cho câu Trắc Nghiệm)
    if (kieuCau === 'TN') {
        ['A', 'B', 'C', 'D'].forEach(da => {
            const lbl = document.getElementById(`lbl_${maCau}_${da}`);
            if (lbl) {
                lbl.style.background = (da === dapAnChon) ? '#e6f4ea' : '#f8f9fa';
                lbl.style.borderColor = (da === dapAnChon) ? '#28a745' : '#dee2e6';
            }
        });
    }

    // 2. Chấm điểm tức thời toàn bộ đề (Dựa theo thuật toán SQL của Thầy)
    let soCauDaLam = 0;
    let soCauDung = 0;
    let tongDiemLive = 0;

    const dsCauHoi = window.HocSinhLiveState.danhSachCauHoi;
    const tongTrongSo = window.HocSinhLiveState.tongTrongSo;

    const diemMaxTN = (10.0 * 1.0) / tongTrongSo;
    const diemMaxTLN = (10.0 * 2.0) / tongTrongSo;
    const diemMaxDS = (10.0 * 4.0) / tongTrongSo;

    dsCauHoi.forEach(cau => {
        const _maCau = cau.ma_cau_hoi || cau.maCau;
        const _dapAnDung = (cau.dap_an || cau.dapAn || '').toUpperCase();
        const _kieuCau = (cau.kieuCau || cau.kieu_cau || 'TN').toUpperCase();
        const _dapAnHs = window.HocSinhLiveState.dapAnHocSinh[_maCau];
        ...
        if (_dapAnHs) {
            ...
            if (_kieuCau === 'TN') {
                if (_dapAnHs === _dapAnDung) { diemCauNay = diemMaxTN; soCauDung++; }
            }
            else if (_kieuCau === 'TLN') {
                if (_dapAnHs === _dapAnDung || _dapAnHs.replace(',', '.') === _dapAnDung.replace(',', '.')) {
                    diemCauNay = diemMaxTLN; soCauDung++;
                }
            }
            else if (_kieuCau === 'DS') {
                let yDung = 0;
                for (let i = 0; i < 4; i++) { if (_dapAnHs[i] && _dapAnDung[i] && _dapAnHs[i] === _dapAnDung[i]) yDung++; }

                if (yDung === 1) diemCauNay = diemMaxDS * 0.10;
                else if (yDung === 2) diemCauNay = diemMaxDS * 0.25;
                else if (yDung === 3) diemCauNay = diemMaxDS * 0.50;
                else if (yDung === 4) { diemCauNay = diemMaxDS * 1.00; soCauDung++; }
            }

            tongDiemLive += diemCauNay;
        }
    });

    // 3. 🚀 BẮN DỮ LIỆU UPDATE LÊN BẢNG tien_do_live_quiz KHÔNG ĐỢI PHẢN HỒI (Lửa cháy ngay màn hình Thầy)
    _supabase.from('tien_do_live_quiz').update({
        so_cau_da_lam: soCauDaLam,
        so_cau_dung: soCauDung,
        diem_so: Number(tongDiemLive.toFixed(2))
        ...
        .eq('uid_hoc_sinh', GocHocSinhState.uid)
        .then(() => { }); // .then rỗng để mã chạy async non-blocking
}

// =====================================================================
// Hàm 8.6.5: Nút Nộp Bài (Lưu vào bảng điểm chính thức)
// =====================================================================
window.ham_8_6_5_nop_bai_live = function () {
    Swal.fire({
        title: 'Chốt kết quả?',
        text: "Bạn có chắc chắn muốn nộp bài? Thành tích của bạn sẽ được ghim lên bảng xếp hạng chung cuộc!",
        icon: 'warning',
        showCancelButton: true,
        confirmButtonText: '🏁 Chắc chắn Nộp!',
        cancelButtonText: 'Hủy',
        confirmButtonColor: '#28a745',
        preConfirm: async () => {
            // Xóa kênh lắng nghe để rảnh máy
            if (window.HocSinhLiveChannel) {
                _supabase.removeChannel(window.HocSinhLiveChannel);
                window.HocSinhLiveChannel = null;
            }

            // Ghi kết quả chính thức vào bảng ket_qua_thi như bình thường
            // (Thầy có thể tái sử dụng Hàm 8.4 nếu cần, ở đây em xử lý thoát Live)
            return true;
        }
    }).then((result) => {
        if (result.isConfirmed) {
            Swal.fire('Hoàn thành xuất sắc! 🎉', 'Em hãy nhìn lên màn hình của Thầy để xem Top 1 thuộc về ai nhé!', 'success')
                .then(() => {
                    // Thoát ra trang chủ học sinh
                    ham_8_1_tai_nhiem_vu_cua_toi(GocHocSinhState.uid, GocHocSinhState.danh_sach_ma_lop, GocHocSinhState.ten);
                });
        }
    });
