...
//         if (nv.loai_nhiem_vu === "Tự luận (Nộp ảnh)") {
//             // Chấm tự luận: Gọi hàm Khối 16, hiển thị icon cây bút
//             htmlNutCham = `<button onclick="ham_16_5_mo_danh_sach_cham_tu_luan('${nv.ma_nhiem_vu}', '${nv.ten_nhiem_vu.replace(/'/g, "\\'")}')" title="Chấm bài tự luận (Xem ảnh học sinh nộp)" style="padding: 6px; background: #e67e22; color: white; border: none; border-radius: 4px; font-weight: bold; cursor: pointer; font-size: 11px; width: 100%; box-shadow: 0 1px 2px rgba(0,0,0,0.1);">✍️ Chấm TL</button>`;
        
//         } else if (nv.loai_nhiem_vu === "Làm đề (Online)" || nv._isLiveTask) {
//             // Chấm trắc nghiệm: Gọi hàm tính điểm tự động, hiển thị icon vòng lặp
//             htmlNutCham = `<button onclick="ham_gv_cham_lai_ca_lop('${nv.ma_nhiem_vu}')" title="Chấm lại toàn bộ điểm trắc nghiệm cho nhiệm vụ này" style="padding: 6px; background: #e67e22; color: white; border: none; border-radius: 4px; font-weight: bold; cursor: pointer; font-size: 11px; width: 100%; box-shadow: 0 1px 2px rgba(0,0,0,0.1);">🔄 Chấm lại TN</button>`;
        
//         } else {
//             // Dự phòng cho các loại nhiệm vụ Khảo sát, Video... (Chưa cần chấm hoặc báo đang phát triển)
//             htmlNutCham = `<button onclick="alert('Tính năng chấm điểm cho nhiệm vụ [${loaiHienThi}] đang được phát triển!')" title="Chưa hỗ trợ chấm điểm" style="padding: 6px; background: #adb5bd; color: white; border: none; border-radius: 4px; font-weight: bold; cursor: not-allowed; font-size: 11px; width: 100%; box-shadow: 0 1px 2px rgba(0,0,0,0.1);">🔒 Chấm #</button>`;
//         }
//         // ====================================================================
...
// // // ==============================================================
// // // Hàm 7.4: Thu thập dữ liệu, Kiểm tra File Giải và Tách Nhiệm Vụ 
// // // (BẢN TÍCH HỢP LIÊN THÔNG VỚI TỰ LUẬN KHỐI 15)
// // // ==============================================================
// // async function ham_7_4_luu_nhiem_vu_moi(btnNode) {
// //     const maNVTrenForm = document.getElementById('add_nv_ma').value;
// //     const tenNV = document.getElementById('add_nv_ten').value.trim();
// //     const loaiNV = document.getElementById('add_nv_loai').value;
// //     let maHL = document.getElementById('add_nv_maHL').value; // Chuyển thành let để có thể ghi đè
// //     const trangThai = document.getElementById('add_nv_trangthai').value;
...
// //     let metadataNhiemVu = {}; // 🌟 CHUẨN BỊ BIẾN CHỨA DỮ LIỆU TỰ LUẬN

// //     // 1. KIỂM TRA ĐẦU VÀO CƠ BẢN
// //     if (!tenNV) return alert("❌ Thầy vui lòng nhập Tên nhiệm vụ!");

// //     // 🌟 MÓC NỐI KHỐI 15: XỬ LÝ RIÊNG NẾU LÀ TỰ LUẬN
// //     if (loaiNV === 'Tự luận (Nộp ảnh)') {
// //         if (typeof ham_15_6_lay_du_lieu_tu_luan_de_luu !== 'function') {
// //             return alert("Lỗi hệ thống: Không tìm thấy phân hệ Tự luận Khối 15!");
// //         }
        
// //         // Gọi hàm Khối 15 (có chữ await vì nó có thể phải nạp Học liệu Text lên DB)
// //         const kqTuLuan = await ham_15_6_lay_du_lieu_tu_luan_de_luu();
// //         if (kqTuLuan.error) return alert("❌ " + kqTuLuan.error); // Dừng lại nếu Khối 15 báo lỗi
        
// //         maHL = kqTuLuan.maHocLieu; 
// //         metadataNhiemVu = kqTuLuan.metadata; 
// //     } else {
// //         // Trắc nghiệm bình thường thì bắt lỗi Học liệu như cũ
// //         if (!maHL || maHL === "KHONG_DUNG") return alert("❌ Thầy chưa chọn Học liệu (Đề thi) kìa!");
// //     }
...
// //                 loai_nhiem_vu: loaiNV,
// //                 ma_hoc_lieu: maHL === "KHONG_DUNG" ? null : maHL,
// //                 metadata: metadataNhiemVu, // 🌟 GHI METADATA CỦA TỰ LUẬN VÀO ĐÂY
// //                 khoi_lop: document.getElementById('add_nv_khoi').value,
// //                 loai_kiem_tra: document.getElementById('add_nv_loaiKT').value,
// //                 quy_mo_cau_hoi: quyMo,
// //                 cau_truc_de: cauTruc,
...
// // ==============================================================
// window.ham_7_4_luu_nhiem_vu_moi = async function(btnNode) {
...
//     try {
//         // 🌟 NHÁNH A: LOGIC CHO NHIỆM VỤ TỰ LUẬN (ĐÃ TÍCH HỢP MÃ HL VÀO TÊN THƯ MỤC)
//     if (loaiNV === 'Tự luận (Nộp ảnh)') {
//         if (typeof ham_15_6_lay_du_lieu_tu_luan_de_luu !== 'function') {
//             throw new Error("Lỗi hệ thống: Không tìm thấy phân hệ Tự luận Khối 15!");
//         }
        
//         btnNode.innerText = "⏳ Đang nạp Học liệu Tự luận...";
//         const kqTuLuan = await ham_15_6_lay_du_lieu_tu_luan_de_luu();
//         if (kqTuLuan.error) throw new Error(kqTuLuan.error); 
        
//         const maHL_TL = kqTuLuan.maHocLieu || "NO_HL"; // Xử lý trường hợp nhiệm vụ Text không có HL
//         const metadataNhiemVu_TL = kqTuLuan.metadata; 

//         let mangPayloadInsert = [];
//         let mangTenThuMucCanTao = [];
//         let mapLopVaTenThuMuc = {}; 
//         const tapKyTu = 'ABCDEFGHJKLMNPQRSTUVWXYZ23456789';
//         const prefix = "NV_" + (CFG_NV.PREFIX_LOAI ? (CFG_NV.PREFIX_LOAI[loaiNV] || "TL") : "TL") + "_";

//         dsLopChon.forEach(maLop => {
//             let maNV_ChinhThuc = (dsLopChon.length === 1 && tinhChat !== "TU_DO") ? maNVTrenForm : prefix + Array(6).fill(0).map(() => tapKyTu.charAt(Math.floor(Math.random() * tapKyTu.length))).join('');
//             let tenLop = mapTenLop[maLop] || maLop;
            
//             // 🌟 CẤU TRÚC TÊN THƯ MỤC: [MÃ HL]-[Mã NV]-[Tên NV]-[Mã Lớp]-[Tên Lớp]
//             if (tinhChat !== "TU_DO") {
//                 // Đảm bảo tên thư mục không chứa ký tự cấm
//                 let tenThuMuc = `${maHL_TL}-${maNV_ChinhThuc}-${tenNV}-${maLop}-${tenLop}`.replace(/[\\/:*?"<>|]/g, ""); 
//                 mangTenThuMucCanTao.push(tenThuMuc);
//                 mapLopVaTenThuMuc[maLop] = tenThuMuc;
//             }

//             mangPayloadInsert.push({
//                 ma_nhiem_vu: maNV_ChinhThuc,
//                 ten_nhiem_vu: tenNV,
//                 loai_nhiem_vu: loaiNV,
//                 ma_hoc_lieu: maHL_TL === "NO_HL" ? null : maHL_TL,
//                 metadata: { ...metadataNhiemVu_TL }, 
//                 khoi_lop: document.getElementById('add_nv_khoi').value,
//                 loai_kiem_tra: document.getElementById('add_nv_loaiKT').value,
//                 danh_sach_lop: [maLop], 
//                 thoi_gian_mo: mo ? new Date(mo).toISOString() : null,
...
//         // GỌI GOOGLE APPS SCRIPT ĐỂ TẠO 1 LOẠT THƯ MỤC DRIVE
...
//             if (ketQuaDrive.status === "success") {
//             console.log("✅ Drive tạo thành công, folderIds:", ketQuaDrive.folderIds);
//             mangPayloadInsert.forEach(payload => {
//                 let mLop = payload.danh_sach_lop[0];
//                 let folderName = mapLopVaTenThuMuc[mLop];
                
//                 // Đảm bảo metadata không bị null hoặc undefined trước khi gán
//                 if (!payload.metadata) payload.metadata = {};
                
//                 if (folderName && ketQuaDrive.folderIds[folderName]) {
//                     // GÁN CHÍNH XÁC VÀO OBJECT PAYLOAD
//                     payload.metadata.folder_id_drive = ketQuaDrive.folderIds[folderName]; 
//                 }
//             });
//         } else {
//             // 🌟 CHỖ NÀY CỰC QUAN TRỌNG: Nếu Google trả về lỗi, nó sẽ văng ra đây
//     console.error("❌ Google Drive trả về lỗi:", ketQuaDrive);
//     throw new Error("Lỗi Google Drive: " + (ketQuaDrive.message || "Không xác định"));
//         }
//         }

//         btnNode.innerText = "⏳ Đang lưu dữ liệu lên máy chủ...";
//         const { error: errInsert } = await _supabase.from('nhiem_vu').insert(mangPayloadInsert);
//         if (errInsert) throw errInsert;

//         alert(`✅ Đã giao bài TỰ LUẬN (${maHL_TL}) thành công cho ${dsLopChon.length} lớp!`);
//         ham_7_1_ve_quan_ly_nhiem_vu();
//     }
//      else {
//             // =========================================================================
