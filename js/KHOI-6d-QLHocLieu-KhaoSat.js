...
// // Hàm 6.0: Sinh mã học liệu ngẫu nhiên (Hỗ trợ tiền tố linh hoạt)
// function ham_6_0_sinh_ma_hoc_lieu(loaiPrefix) {
...
//     // Nếu tiền tố đã có dấu gạch dưới (như TN_GK) thì dùng luôn
//     // Nếu là mã cũ DE thì trả về HL_DE_...
//     const prefix = loaiPrefix.includes('_') ? loaiPrefix : loaiPrefix.replace('_', '');
    
//     // Đảm bảo cấu trúc: HL_LOAI_RANDOM
//     return `HL_${prefix}_${randomPart}`;
// }
...
//                         <optgroup label="🎯 1. NHIỆM VỤ: LÀM ĐỀ TRẮC NGHIỆM">
...
//                         <optgroup label="✍️ 2. NHIỆM VỤ: LÀM BÀI TỰ LUẬN">
//                             <option value="TL_PDF">Tự luận - Đính kèm PDF</option>
//                             <option value="TL_WORD">Tự luận - Đính kèm Word</option>
//                             <option value="TL_IMG">Tự luận - Đính kèm Ảnh</option>
//                             <option value="TL_TXT">Tự luận - Văn bản trực tiếp</option>
//                             <option value="TL_PBT">Phiếu bài tập / BTVN</option>
//                         </optgroup>
                        
//                         <optgroup label="📺 3. NHIỆM VỤ: XEM BÀI / LÝ THUYẾT">
//                             <option value="BG_VID">Bài giảng - Video</option>
//                             <option value="BG_SLD">Bài giảng - Slide trình chiếu</option>
//                             <option value="BG_PRE">Tài liệu đọc trước (Lớp học đảo ngược)</option>
//                             <option value="BG_STM">Hướng dẫn thực hành STEM</option>
//                         </optgroup>
                        
//                         <optgroup label="📊 4. NHIỆM VỤ: KHẢO SÁT & Ý KIẾN">
//                             <option value="KS_FRM">Khảo sát qua Biểu mẫu (Link Form)</option>
//                             <option value="KS_DG">Phiếu đánh giá tiếp thu / Góp ý</option>
//                         </optgroup>
//                     </select>
//                 </div>
...
//         }
//     });
//     setTimeout(() => ham_6_3_b_cap_nhat_ma_theo_loai(), 50);
// }
...
// ==============================================================
window.ham_6_3_hien_form_them_hoc_lieu = function () {
    const vungLamViec = document.getElementById('vung-lam-viec-chi-tiet');
    const maHLBanDau = ham_6_0_sinh_ma_hoc_lieu('TN_GK1');
        ...
        <div style="max-width: 950px; background: #ffffff; padding: 25px; border-radius: 12px; border: 1px solid #e0e0e0; margin: 0 auto; box-shadow: 0 4px 15px rgba(0,0,0,0.1);">
            ...
            <div id="vung_cau_hinh_chi_tiet">
                <div id="khu_vuc_nhap_trac_nghiem" style="display: block;">
                    ...
                    <div id="khu_vuc_cach_1" style="display: block; margin-bottom: 20px; padding: 15px; border: 1px solid #17a2b8; border-radius: 8px; background: #e8f4fd;">
                        <div style="display: flex; gap: 15px; align-items: center; flex-wrap: wrap;">
                            <button onclick="ham_6_15_tai_file_mau()" style="padding: 10px 15px; background: #6c757d; color: white; border: none; border-radius: 6px; font-weight: bold; cursor: pointer;">📄 Tải File Mẫu</button>
                            <div style="flex: 1; min-width: 250px;">
                                <input type="file" id="upload_file_input" accept=".tex,.txt" onchange="ham_6_16_xu_ly_thay_doi_file(this)" style="display: block; width: 100%; padding: 8px; border: 2px dashed #17a2b8; border-radius: 6px; background: #fff; cursor: pointer;">
                            </div>
                            <button id="btn_check_file" onclick="ham_6_13_kiem_tra_file_upload()" style="padding: 10px 15px; background: #ffc107; color: #333; border: none; border-radius: 6px; font-weight: bold; cursor: pointer; display: none;">🔍 Kiểm tra</button>
                        </div>
                        <div id="khu_vuc_bao_loi_file" style="margin-top: 10px; display: none; padding: 10px; background: #fff3cd; border-left: 4px solid #ffc107; font-size: 13px;"></div>
                    </div>
                    ...
                    <div id="khu_vuc_cach_2" style="display: none; margin-bottom: 20px; padding: 15px; border: 1px dashed #e67e22; border-radius: 8px; background: #fffaf0;">
                        ...
                        <div style="display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 12px;">
                            <div>
                                ...
                                <textarea id="txtID_TN" oninput="ham_6_5_tinh_toan_cau_truc()" rows="7" style="width: 100%; padding: 8px; border: 1px solid #1a73e8; border-radius: 4px;"></textarea>
                            </div>
                            <div>
                                <label style="font-size: 11px; font-weight: bold; color: #d35400; display: block;">PHẦN 2: ĐÚNG / SAI (<span id="demDS">0</span>)</label>
                                <textarea id="txtID_DS" oninput="ham_6_5_tinh_toan_cau_truc()" rows="7" style="width: 100%; padding: 8px; border: 1px solid #d35400; border-radius: 4px;"></textarea>
                            </div>
                            <div>
                                <label style="font-size: 11px; font-weight: bold; color: #28a745; display: block;">PHẦN 3: TRẢ LỜI NGẮN (<span id="demTLN">0</span>)</label>
                                <textarea id="txtID_TLN" oninput="ham_6_5_tinh_toan_cau_truc()" rows="7" style="width: 100%; padding: 8px; border: 1px solid #28a745; border-radius: 4px;"></textarea>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div style="display: flex; gap: 12px; margin-top: 20px;">
                <button id="btnLuuHocLieu" onclick="ham_6_4_luu_hoc_lieu_moi(this)" style="flex: 2; padding: 12px; background: #28a745; color: white; border: none; border-radius: 6px; font-weight: bold; cursor: pointer; font-size: 15px;">
                    💾 LƯU HỌC LIỆU
                </button>
                <button onclick="ham_6_1_ve_quan_ly_hoc_lieu()" style="flex: 1; padding: 12px; background: #6c757d; color: white; border: none; border-radius: 6px; cursor: pointer; font-weight: bold; font-size: 15px;">
                    ❌ HỦY
...
window.ham_6_3_b_cap_nhat_ma_theo_loai = function () {
    ...
    // 1. Cập nhật mã
    txtMa.value = ham_6_0_sinh_ma_hoc_lieu(loaiChon);

    // 2. Định tuyến hiển thị
    if (loaiChon.startsWith('TN_')) {
        // --- LUỒNG TRẮC NGHIỆM ---
        kvTracNghiem.style.display = 'block';
        vungNhungK15.style.display = 'none';
        vungNhungK15.innerHTML = "";
    } 
    else if (loaiChon.startsWith('KS_')) {
        // --- LUỒNG KHẢO SÁT (Để trống hoặc hiển thị thông báo) ---
        kvTracNghiem.style.display = 'none';
        vungNhungK15.style.display = 'block';
        vungNhungK15.innerHTML = `
            <div style="padding: 30px; text-align: center; border: 2px dashed #ccc; border-radius: 8px; color: #888;">
                <p>⚙️ <b>Phân hệ Khảo sát đang được phát triển.</b></p>
                <p>Vui lòng quay lại sau!</p>
            </div>
        `;
    } 
    else {
        // --- LUỒNG TỰ LUẬN / BÀI GIẢNG (GỌI KHỐI 15) ---
        kvTracNghiem.style.display = 'none';
        vungNhungK15.style.display = 'block';
        
        if (typeof window.ham_15_11_html_form_hoc_lieu_khac === 'function') {
            vungNhungK15.innerHTML = window.ham_15_11_html_form_hoc_lieu_khac(loaiChon);
        } else {
            vungNhungK15.innerHTML = `<p style="color:red;">Lỗi: Không tìm thấy phân hệ Khối 15.</p>`;
        }
    }
};
...
//     // 🌟 Tự động lấy "Text" của tuỳ chọn (VD: "Trắc nghiệm - Giữa kỳ (GK1, GK2)") làm Loại kiểm tra
...
window.ham_6_4_luu_hoc_lieu_moi = async function (btn) {
    ...
    // 🌟 ĐỊNH TUYẾN LUỒNG XỬ LÝ (QUAN TRỌNG)
    const isTracNghiem = phanLoai.startsWith('TN_')|| phanLoai === 'DE';
    ...
    if (isTracNghiem) {
        ...
        if (dangChonCach1) {
            ...
            const parseTay = (text, kieu) => {
                ...
                lines.forEach(line => {
                    danhSachKhoBau.push({ dap_an: "", ma_goc: taoMaGoc(), ma_cau_hoi: taoMaCauHoi(), ma_loi_giai: taoMaLoiGiai() });
                });
            }
            parseTay(document.getElementById('txtID_TN').value, "TN");
            parseTay(document.getElementById('txtID_DS').value, "DS");
        ...
        // --- LUỒNG 2: TỰ LUẬN / BÀI GIẢNG (GỌI KHỐI 15) ---
        if (typeof window.ham_15_12_luu_hoc_lieu_khac === 'function') {
            await window.ham_15_12_luu_hoc_lieu_khac(maHL, tenHL, phanLoai, loaiKiemTra, khoiLop, thoiGian, trangThai, btn);
        } else {
            Swal.fire('Lỗi', 'Phân hệ Tự luận (Khối 15) chưa sẵn sàng!', 'error');
        }
    }
};
...
window.ham_6_6_mo_form_sua_hoc_lieu = async function (maHocLieu, choPhepSua = true) {
    ...
    // 🌟 CHỐT CHẶN DÒ TÌM PHÂN LOẠI KHẲNG ĐỊNH ĐÚNG Ý THẦY
    let loaiHL = "";
    if (maHocLieu.includes('_TN_') || maHocLieu.includes('_DE_')) {
        loaiHL = "TRAC_NGHIEM";
    } else if (maHocLieu.includes('_TL_')) {
        loaiHL = "TU_LUAN";
    } else if (maHocLieu.includes('_BG_')) {
        loaiHL = "BAI_GIANG";
    } else if (maHocLieu.includes('_KS_')) {
        loaiHL = "KHAO_SAT";
    }

    // Hiển thị trạng thái chờ tùy biến theo loại
    if (loaiHL === "TRAC_NGHIEM") {
        ...
        vungLamViec.innerHTML = `<div style="text-align: center; padding: 40px;"><div style="border: 4px solid #f3f3f3; border-top: 4px solid #6f42c1; border-radius: 50%; width: 40px; height: 40px; animation: spin 1s linear infinite; margin: 0 auto;"></div><h3 style="color:#6f42c1; margin-top:15px;">⏳ Đang nạp thông tin học liệu...</h3></div>`;
    }

    let dsCauHoiGithub = [];
    let urlFileGiai = data.url_file_giai || "";

    // Chỉ kết nối API GitHub nếu đây là luồng Trắc nghiệm
    if (loaiHL === "TRAC_NGHIEM") {
        try {
            if (data.url_github) {
    ...
    if (choPhepSua) {
        if (loaiHL === "TRAC_NGHIEM") mauTieuDe = "#f39c12";
        else if (loaiHL === "TU_LUAN") mauTieuDe = "#6f42c1";
        else if (loaiHL === "BAI_GIANG") mauTieuDe = "#28a745";
        else if (loaiHL === "KHAO_SAT") mauTieuDe = "#e74c3c";
    }
    ...
    // 🌟 PHÂN LUỒNG HIỂN THỊ HOÀN TOÀN BẰNG ĐIỀU KIỆN "CÓ"
    if (loaiHL === "TU_LUAN" || loaiHL === "BAI_GIANG") {
        // Gọi phân hệ Khối 15 VÀ TRUYỀN QUYỀN SỬA SANG
        if (typeof window.ham_15_13_render_xem_hoc_lieu_khac === 'function') {
            
            // SỬA DÒNG NÀY (Thêm biến choPhepSua vào đuôi)
            htmlCards = window.ham_15_13_render_xem_hoc_lieu_khac(data, choPhepSua); 
            
        } else {
            htmlCards = `<p style="text-align:center; color:red; padding: 20px;">⚠️ Lỗi: Phân hệ Khối 15 chưa sẵn sàng.</p>`;
        }
    }
    else if (loaiHL === "KHAO_SAT") {
        // Vùng trống dự phòng cho Khảo sát sau này phát triển tiếp
        htmlCards = `
            <div style="padding: 35px; text-align: center; border: 2px dashed #e74c3c; border-radius: 8px; background: #fffbfb; color: #555;">
                <p>📊 <b>Đây là phân hệ dữ liệu Khảo sát & Ý kiến học sinh.</b></p>
                <p style="font-size:13px; color:#777;">Hệ thống đang lưu trữ và quản lý cấu trúc liên kết biểu mẫu.</p>
                ${data.url_github ? `<p>Đường dẫn Form: <a href="${data.url_github}" target="_blank" style="color:#e74c3c; font-weight:bold; text-decoration:underline;">Bấm vào đây để kiểm tra Link</a></p>` : ''}
            </div>`;
    } 
    else if (loaiHL === "TRAC_NGHIEM") {
    ...
    vungLamViec.innerHTML = `
        <div style="max-width: 1000px; background: white; padding: 25px; border-radius: 12px; border: 1px solid #e0e0e0; margin: 0 auto;">
            ...
            <div style="display: flex; gap: 10px;">
                ${choPhepSua ? `<button onclick="ham_6_7_luu_cap_nhat_hoc_lieu('${maHocLieu}', this)" style="padding:15px; background:#28a745; color:white; border:none; border-radius:6px; font-weight:bold; cursor:pointer; flex:2;">💾 LƯU THAY ĐỔI ${loaiHL === "TRAC_NGHIEM" ? 'VÀ ĐỒNG BỘ GITHUB' : ''}</button>` : ''}
...
// // ==============================================================
// // Hàm 6.7: Lưu Cập Nhật Học Liệu (Ủy quyền phân nhánh khẳng định)
// // ==============================================================
...
//     // 🌟 ĐỊNH TUYẾN LUỒNG KIỂM TRA BẰNG ĐIỀU KIỆN "CÓ"
//     let loaiHL = "";
//     if (maHocLieu.includes('_TN_') || maHocLieu.includes('_DE_')) {
//         loaiHL = "TRAC_NGHIEM";
//     } else if (maHocLieu.includes('_TL_')) {
...
//     try {
//         if (loaiHL === "TRAC_NGHIEM") {
//             // --- LUỒNG XỬ LÝ ĐỀ TRẮC NGHIỆM (ĐẨY FILE LÊN GITHUB) ---
...
//             if (banDoSupabase.length === 0) throw new Error("Không thể lưu học liệu trắc nghiệm rỗng!");
...
//             if (chkGopFile && chkGopFile.checked && typeof window.ham_7_10_ra_lenh_tao_file_giai === 'function') {
//                 btnNode.innerText = "⏳ ĐANG GOM FILE GIẢI (JSON)...";
//                 await window.ham_7_10_ra_lenh_tao_file_giai(null, maHocLieu);
//             }
//         } else {
//             // --- LUỒNG XỬ LÝ CÁC HỌC LIỆU KHÁC (TỰ LUẬN, BÀI GIẢNG, KHẢO SÁT) ---
//             // Bỏ qua toàn bộ mảng câu hỏi và đồng bộ GitHub, chỉ cập nhật thông tin vỏ bọc lý lịch
//             btnNode.innerText = "⏳ ĐANG LƯU VÀO DATABASE...";
//             await _supabase.from('hoc_lieu').update({
//                 ten_hoc_lieu: tenMoi, 
...
window.ham_6_7_luu_cap_nhat_hoc_lieu = async function (maHocLieu, btnNode) {
    ...
    // 🌟 2. ĐỊNH TUYẾN LUỒNG KIỂM TRA BẰNG ĐIỀU KIỆN "CÓ"
    let loaiHL = "";
    if (maHocLieu.includes('_TN_') || maHocLieu.includes('_DE_')) {
        loaiHL = "TRAC_NGHIEM";
    } else if (maHocLieu.includes('_TL_')) {
    ...
    try {
        if (loaiHL === "TRAC_NGHIEM") {
            // =================================================================
            // LUỒNG 1: XỬ LÝ ĐỀ TRẮC NGHIỆM (CẬP NHẬT CÂU HỎI & ĐỒNG BỘ GITHUB)
            // =================================================================
            const cards = document.querySelectorAll('.card-cau-hoi');
            let banDoSupabase = [];
            let dsCauHoiGithubMoi = [];
            ...
            // =================================================================
            // LUỒNG 2: XỬ LÝ HỌC LIỆU KHÁC (TỰ LUẬN, BÀI GIẢNG, KHẢO SÁT)
            // =================================================================
            btnNode.innerText = "⏳ ĐANG LƯU VÀO DATABASE...";
            ...
            // Tìm lại dữ liệu cũ trong State để không làm mất các thuộc tính metadata khác
            const culaDataCu = BangHocLieuState.duLieu.find(hl => hl.ma_hoc_lieu === maHocLieu);
            

            // 1. Parse metadata cũ ra thành Object
let metaObj = typeof culaDataCu.metadata === 'string' ? JSON.parse(culaDataCu.metadata) : (culaDataCu.metadata || {});

// 2. CẬP NHẬT: Luôn lấy giá trị từ select mức độ đáp án (dù là tạo mới hay sửa)
// Chúng ta lấy theo ID của select trong giao diện sửa
const elMucDo = document.getElementById('select_muc_do_dapan_k15');
if (elMucDo) {
    metaObj.cau_hinh_dap_an = { 
        muc_do: parseInt(elMucDo.value) 
    };
}

// 3. Nếu người dùng đang sửa học liệu dạng Văn bản trực tiếp (TL_TXT)
const elTextAreaText = document.getElementById('sua_noi_dung_txt_k15');
if (elTextAreaText) {
    metaObj.noi_dung_chinh = elTextAreaText.value; 
}

// 4. Đưa metadata đã cập nhật vào payload
updatePayload.metadata = metaObj;



    //   // Nếu người dùng đang sửa học liệu dạng Văn bản trực tiếp (TL_TXT)
    //         if (elTextAreaText && culaDataCu) {
    //             // Parse metadata cũ ra thành Object
    //             let metaObj = typeof culaDataCu.metadata === 'string' ? JSON.parse(culaDataCu.metadata) : (culaDataCu.metadata || {});
                
    //             // Ghi đè nội dung chữ mới vào thuộc tính noi_dung_chinh
    //             metaObj.noi_dung_chinh = elTextAreaText.value; 
                
    //             // Cập nhật lại vào gói payload
    //             updatePayload.metadata = metaObj;
    //         }

            // Thực thi cập nhật lên Supabase (chỉ cập nhật thông tin vỏ bọc + metadata nếu có)
        ...
        if (typeof window.ham_6_1_ve_quan_ly_hoc_lieu === 'function') {
            window.ham_6_1_ve_quan_ly_hoc_lieu();
        }
...
window.ham_6_16_xu_ly_thay_doi_file = function(input) {
...
window.ham_6_20_xoa_sach_github_va_supabase = async function (maHL, btnNode) {
    ...
    try {
        ...
        if (isTracNghiem) {
            ...
            // =================================================================
            // LUỒNG B: XÓA TỰ LUẬN / KHÁC (ỦY QUYỀN KHỐI 15)
            // =================================================================
            btnNode.innerText = "⏳ Đang dọn dẹp tài nguyên đính kèm...";
            if (typeof window.ham_15_14_xoa_tai_nguyen_tu_luan === 'function') {
                await window.ham_15_14_xoa_tai_nguyen_tu_luan(hocLieu);
            }
        }
