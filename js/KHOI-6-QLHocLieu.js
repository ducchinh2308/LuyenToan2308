...
// ==============================================================
// [Nhãn thời gian: Mới nhất] - Hàm 6.6: Vẽ Form Sửa Học Liệu (Chống Cache Tuyệt Đối Bằng API)
// ==============================================================
window.ham_6_6_mo_form_sua_hoc_lieu = async function (maHocLieu, choPhepSua = true) {
    const data = BangHocLieuState.duLieu.find(hl => hl.ma_hoc_lieu === maHocLieu);
    if (!data) return alert("Dữ liệu không tồn tại!");
    ...
    const vungLamViec = document.getElementById('vung-lam-viec-chi-tiet');
    vungLamViec.innerHTML = `<div style="text-align: center; padding: 40px;"><div style="border: 4px solid #f3f3f3; border-top: 4px solid #3498db; border-radius: 50%; width: 40px; height: 40px; animation: spin 1s linear infinite; margin: 0 auto;"></div><h3 style="color:#1a73e8; margin-top:15px;">⏳ Đang tải nội dung thời gian thực từ kho GitHub...</h3></div>`;
    ...
    try {
        if (data.url_github) {
            let fetchUrl = data.url_github + "?t=" + Date.now();
            let fetchOptions = { cache: 'no-store' }; // Ép trình duyệt không dùng cache

            // 🌟 CÔNG NGHỆ CHỐNG CACHE TUYỆT ĐỐI: Dùng GitHub API thay vì Github Pages / Raw
            if (typeof CFG_HE_THONG !== 'undefined' && data.url_github.includes('.io/')) {
                try {
                    const urlParts = data.url_github.split('.io/');
                    let pathParts = urlParts[1].split('/');
                    ...
                    fetchUrl = `https://api.github.com/repos/${CFG_HE_THONG.GITHUB_REPO}/contents/${pathFileDe}`;
                    fetchOptions = {
                        method: 'GET',
                        headers: {
                            "Authorization": `token ${CFG_HE_THONG.GITHUB_TOKEN}`,
                            "Accept": "application/vnd.github.v3.raw", // Ép GitHub trả về nội dung File thuần, bỏ qua mọi lớp Cache CDN
                            "Cache-Control": "no-cache"
                        },
                        cache: 'no-store'
                    };
                    console.log("🔗 [DEBUG FETCH] Đang kéo file thời gian thực qua API:", fetchUrl);
                } catch (e) { }
            }
            ...
            if (res.ok) {
                ...
                console.warn("Lỗi 404: Không tìm thấy file trên Github API");
            }
        }
    } catch (e) {
        console.error("Lỗi mạng khi nạp file:", e);
